...
type InventorySaveAction = 'done' | 'add-another';
interface InventoryEditorProps { item?:InventoryItem; initialDraft?:InventoryDraft; assisted?:boolean; assistedWarnings?:string[]; locationSuggestions?:string[]; onCancel():void; onSave(draft:InventoryDraft,action?:InventorySaveAction):Promise<void>; }
...
export function InventoryEditor({ item, initialDraft, assisted=false, assistedWarnings=[], locationSuggestions=[], onCancel, onSave }: InventoryEditorProps) {
  ...
  async function submit(action:InventorySaveAction='done'){if(!draft.itemName.trim()||!draft.category.trim()||!draft.location.trim()){setError('Item name, category, and location are required.');return;}if((draft.userEnteredValue!==undefined&&!Number.isFinite(draft.userEnteredValue))||(draft.purchasePrice!==undefined&&!Number.isFinite(draft.purchasePrice))){setError('Purchase price and manual value must be valid numbers.');return;}setError('');setSaving(true);try{await onSave(draft,action);}catch{setError('This item could not be saved. Please try again.');}finally{setSaving(false);}}
  return <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={onCancel}><ScrollView style={styles.safe} contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    {assisted?<View style={styles.assist}><Text style={styles.assistTitle}>AI-prefilled draft</Text><Text style={styles.assistCopy}>Review the highlighted identity details before saving. Always compare the serial number with the label on the physical item.</Text>{assistedWarnings.map(warning=><Text key={warning} style={styles.assistWarning}>• {warning}</Text>)}</View>:null}
    ...
    {!item?<Pressable accessibilityRole="button" style={styles.secondarySave} disabled={saving} onPress={()=>void submit('add-another')}><Text style={styles.secondarySaveText}>{saving?'Saving…':assisted?'Save & photograph another':'Save & add another'}</Text></Pressable>:null}
