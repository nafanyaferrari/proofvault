...
import { archiveInventoryItem, getLatestValuation, getSubscriptionTier, initializeDatabase, listArchivedInventory, listInventory, restoreInventoryItem, saveInventoryItem, saveItemAttachment, saveValuation, setSubscriptionTier } from './src/db/inventoryRepository';
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    const [inventory,archived,subscriptionTier,savedIncidents]=await Promise.all([listInventory(db),listArchivedInventory(db),getSubscriptionTier(db),listIncidents(db)]);
    setItems(inventory);setArchivedItems(archived);setTier(subscriptionTier);setIncidents(savedIncidents);
    setLoading(false);
  }, []);
