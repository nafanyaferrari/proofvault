...
export function ItemForm({ item, assisted = false, assistedWarnings = [], locations, tier, onUpgrade, onCancel, onSave, onSaveAndAddAnother }: ItemFormProps) {
  ...
  const suggest=async()=>{if(!isPremium){setError('AI description assistance is a Premium feature. Free users can still write their own description.');return;}setAiLoading(true);const result=await aiDescriptionService.suggest({photos:draft.photos,notes:draft.notes,category:draft.category,make:draft.make,model:draft.model,serialNumber:draft.serialNumber});setAiResult(result);set('aiSuggestedTitle',result.suggestedTitle);set('aiDescription',result.suggestedDescription);setAiLoading(false)};
  ...
  return <form className="itemForm" onSubmit={submit}>
    ...
    <div className="formGrid">
      ...
      <section className="panel formSection aiSection"><div className="aiHeading"><div><p className="eyebrow green">{isPremium?'SUGGESTED BY AI — PLEASE VERIFY':'PREMIUM AI ASSIST'}</p><h2>AI description assistant</h2></div><button type="button" onClick={isPremium?suggest:onUpgrade} disabled={aiLoading}>{isPremium?<Sparkles/>:<LockKeyhole/>}{aiLoading?'Reviewing…':isPremium?'Generate suggestion':'Upgrade for AI suggestions'}</button></div><p className="helper">{isPremium?'Premium can draft descriptions from photos and visible details. AI output is never treated as a confirmed identifier.':'Free users can still enter descriptions, make/model, serial numbers, and evidence manually. Premium unlocks AI-generated descriptions and identifier help.'}</p>{aiResult&&<div className="aiResult"><b>{aiResult.suggestedTitle}</b><p>{aiResult.suggestedDescription}</p>{aiResult.visibleIdentifiers.map(identifier=><small key={identifier}>{identifier}</small>)}{aiResult.missingRecommendedFields.length>0&&<small>Recommended: add {aiResult.missingRecommendedFields.join(', ')}.</small>}<div><button type="button" onClick={()=>{set('itemName',aiResult.suggestedTitle);set('userDescription',aiResult.suggestedDescription)}}>Use title & description</button></div></div>}</section>
    </div>
